...
impl UploadManager {
    ...
    pub fn new(base_url: String, config: UploadConfig, state_dir: PathBuf) -> Result<Self> {
        ...
        let protocol = if headers.is_empty() {
            TusProtocol::new(base_url, Duration::from_secs(config.timeout))?
        } else {
            TusProtocol::with_headers(base_url, Duration::from_secs(config.timeout), headers)?
        };
    ...
    pub async fn upload_file(&self, file_path: &Path) -> Result<()> {
        ...
        // Get file metadata
        let file_size = std::fs::metadata(file_path).map_err(ZtusError::from)?.len();
        ...
        tracing::info!(
            "Starting upload from offset: {} / {} bytes",
            offset,
            file_size
        );
        ...
        let (mut current_chunk_size, mut throughput_history) = if self.config.adaptive.enabled {
            ...
            tracing::info!(
                ...
                self.config.adaptive.initial_chunk_size / 1024 / 1024
            );
            tracing::info!(
                "Chunk size range: {} MB - {} MB",
                self.config.adaptive.min_chunk_size / 1024 / 1024,
                self.config.adaptive.max_chunk_size / 1024 / 1024
            );
            (
                self.config.adaptive.initial_chunk_size,
                Vec::with_capacity(5),
            )
        } else {
        ...
        loop {
            ...
            if self.config.verbose && n < current_chunk_size && offset + (n as u64) < file_size {
                tracing::debug!(
            ...
            let new_offset = match self
                .protocol
                .upload_chunk_with_retry(&upload_url, offset, chunk, self.config.max_retries)
                .await
            {
            ...
            if self.config.adaptive.enabled && duration_secs > 0.0 {
                    ...
                    && throughput_history.len() >= 2
                {
                    ...
                    let variance: f64 = throughput_history
                        .iter()
                        .map(|&t| (t - avg_throughput).abs() / avg_throughput)
                        .sum::<f64>()
                        / throughput_history.len() as f64;
                    ...
                    if variance > self.config.adaptive.stability_threshold {
                        ...
                        let first_half_avg: f64 = throughput_history
                            .iter()
                            .take(throughput_history.len() / 2)
                            .sum::<f64>()
                            / (throughput_history.len() / 2) as f64;
                        let second_half_avg: f64 = throughput_history
                            .iter()
                            .skip(throughput_history.len() / 2)
                            .sum::<f64>()
                            / (throughput_history.len() - throughput_history.len() / 2) as f64;
                        ...
                        if throughput_change > self.config.adaptive.stability_threshold {
                            ...
                            current_chunk_size =
                                (current_chunk_size * 2).min(self.config.adaptive.max_chunk_size);
                            ...
                            current_chunk_size =
                                (current_chunk_size / 2).max(self.config.adaptive.min_chunk_size);
                            if current_chunk_size != old_chunk_size {
                                tracing::info!(
                        ...
                        if avg_throughput < MIN_EXPLORATION_THROUGHPUT
                            && current_chunk_size < self.config.adaptive.max_chunk_size
                        {
            ...
            if file_size > 0 {
                ...
                if percent > last_percent {
                    ...
                    tracing::info!("Progress: {}% ({} / {} bytes)", percent, offset, file_size);
                }
            }
    ...
    async fn start_new_upload(
        ...
        if let Ok(caps) = &capabilities {
            ...
            if !caps
                .extensions
                .contains(&crate::protocol::TusExtension::Creation)
            {
                tracing::warn!(
            ...
            if self.config.verify_checksum
                && !caps
                    .extensions
                    .contains(&crate::protocol::TusExtension::Checksum)
            {
                tracing::warn!(
            ...
            .await
        {
                        ...
                        || error_msg.contains("checksum not supported")) =>
            {
                ...
                if checksum_algo == Some(crate::config::ChecksumAlgorithm::Sha1) {
                    ...
                    let sha256_checksum = calculate_file_checksum(
                        file_path,
                        crate::config::ChecksumAlgorithm::Sha256,
                    )?;
                    tracing::info!("Calculated sha256 checksum: {}", sha256_checksum);
                    ...
                    self.protocol
                        .create_upload(file_size, Some(self.config.metadata.clone()), None, None)
                        .await?
                }
            }
            Err(e) => return Err(e),
        ...
        let state = UploadState::new(file_path.to_path_buf(), upload_url.clone(), file_size);
